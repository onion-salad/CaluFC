...
serve(async (req)=>{
  ...
  try {
    ...
    // ★★★ Swiftアプリからは image_path (相対パス) または image_data (base64) を受け取る ★★★
    const { image_path, image_data, user_id, eaten_at } = body;
    if ((!image_path && !image_data) || !user_id) {
      throw new Error(`Required parameters missing: ${!image_path && !image_data ? 'image_path or image_data ' : ''}${!user_id ? 'user_id' : ''}`);
    }
    console.log('📅 Eaten at:', eaten_at || 'Not specified (will use current time)');
    console.log('🖼️ Image source:', image_data ? 'base64 data' : 'storage path');

    let signedImageUrl: string;

    if (image_data) {
      // Base64データの場合は直接data URIとして使用
      console.log('🔧 Using base64 image data');
      signedImageUrl = `data:image/jpeg;base64,${image_data}`;
    } else {
      // Storage pathの場合は従来通りSigned URLを生成
      console.log('🔧 Using storage path, generating signed URL');
      // --- Signed URL の生成 ---
    ...
    return new Response(JSON.stringify({
      ...
      data: {
        ...analysis,
        image_url: image_path || 'base64_data',
